...
this.onRun = async ({ api,send, event, Users }) => {
        const ping = Date.now();
...
📝 Dấu lệnh mặc định: ${global.settings.PREFIX}
...
👤 Yêu cầu bởi: ${name}
  `.trim();
            api.sendMessage({body:replyMsg,attachment: queues.splice(0, 1)}, event.threadID, event.messageID);
        } catch (error) {
