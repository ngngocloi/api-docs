...
let isFB = url => /^http(|s):\/\/([^]*)(facebook|fb)\.(com|watch)\/(share\/[^\/]+|[^\/]*)\//.test(url);
...
exports.methods = {
  get: async(req, res, next) => {
    ...
    try {
      ...
      let data = cache[url] || await getInfo(url);
      cache[url] = data;
      ...
      if (/^info-post$/.test(type)) {
        ...
        if (/(\/reel\/|watch)/.test(url)) {
          if (_.attachments.length > 0 && typeof _.attachments?.[0]?.[0]?.media == 'object') repData.attachment.push(_.attachments?.[0]?.[0]?.media || _.playback_video?.[0]);
          else if (_.video.length > 0) repData.attachment.push((_.video[0].__typename = 'Video', _.video[0]) || _.playback_video?.[0]);
...
function getInfo(url) {
  return fetch(url, {
    headers: {
      ...
      'Sec-Fetch-User': '?1',
      "encoding": "gzip",
      "cookie": "datr=CRJhZwPQ6hnXib6UtjDjI83N;sb=CRJhZ3TPISfuLb0Z5h3RZZ9R;ps_l=1;ps_n=1;dpr=0.8999999761581421;c_user=1247419593;fbl_st=100633669%3BT%3A28957864;wl_cbv=v2%3Bclient_version%3A2716%3Btimestamp%3A1737471880;wd=1468x757;ar_debug=1;fr=1D1EMJz6uFKIP8zBU.AWVUJWLE-kklZzI0dNLtAFaS6lVh1V81rP1uYA.Bno3xV..AAA.0.0.Bno3xV.AWWujPg5b6s;xs=31%3ASsEH2Hf3HXQhzA%3A2%3A1735727015%3A-1%3A3523%3A%3AAcV8k-Tlan6gRDxXh8gDxYYesVZHi21cHd9aMHVofZk;presence=C%7B%22lm3%22%3A%22g.8867281286641611%22%2C%22t3%22%3A%5B%7B%22o%22%3A0%2C%22i%22%3A%22g.8826064214170387%22%7D%5D%2C%22utc3%22%3A1738767459799%2C%22v%22%3A1%7D;",
