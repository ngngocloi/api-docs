...
const videoFolderPath = path.join(__dirname, '../cos'); // Đường dẫn tới thư mục chứa video
...
const getRandomVideo = (req, res) => {
    fs.readdir(videoFolderPath, (err, files) => {
        ...
        const videoUrl = `${req.protocol}://${req.get('host')}/cos/${randomFile}`;
...
module.exports = {
    info: {
        path: '/cos',
        title: 'Random Video cos ',
        desc: 'Get a random video URL from the video folder',
        example_url: [
            { method: 'get', query: '/cos', desc: 'Get a random video URL' }
        ]
    },
    methods: {
        get: getRandomVideo
